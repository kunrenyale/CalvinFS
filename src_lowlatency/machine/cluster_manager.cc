...
void ClusterManager::Update() {
  ...
  for (map<uint64, MachineInfo>::const_iterator it =
    ...
    string* ssh_command = new string(
      ...
      " 'cd " + calvin_path_ + "; git pull; cd src_lowlatency; cp Makefile.default Makefile; make clean; make -j'");
    pthread_create(
        &threads[threads.size()-1],
        NULL,
        SystemFunction,
...
void ClusterManager::DeployCluster(double time, int experiment) {
  vector<pthread_t> threads;
  ...
  for (map<uint64, MachineInfo>::const_iterator it =
    ...
    string* ssh_command = new string(
         ...
         "  --config=" + config_file_ + " --time=" + DoubleToString(time) + " --experiment=" + IntToString(experiment) + " ' &");
    pthread_create(
        &threads[threads.size()-1],
        NULL,
        SystemFunction,
